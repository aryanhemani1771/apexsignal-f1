...
- **Phase:** 4 (News intelligence) — **COMPLETE** (structured event moves a prior; telemetry confirms/rejects).
- **Last agent:** Claude Code (Opus 4.8), 2026-07-19.
- **Next action:** start **Phase 5** — prediction-market integration: unified `MarketDataAdapter`;
  Kalshi public + WebSocket + demo execution (`LiveTradingDisabledError` guard); Polymarket
  read-only (+ optional geo check, graceful disable); a synthetic market adapter that always
  works offline; rule-aware contract mapping with a confidence threshold + manual-review gate;
  and an opportunity dashboard page. Compare Phase 3 model prices to market books.
- **Deferred (non-blocking):** DuckDB/Parquet backends; LightGBM GBM baseline (D-009); widen
  Phase 2 evaluation to multiple seasons; adapter should emit explicit `DriverRetired` events
  (D-011); latent-pace particle filter; optional hosted-LLM extractor (deterministic rule-based
  shipped; Protocol in place — D-012); real FIA/RSS/GDELT news adapters (fixture + rule-based
  pipeline shipped); richer bundled demo race for pricing; dashboard not run in CI.
